...
export default function ReviewMainForm({ initialValues }) {
  const editorRef = useRef(null);
  // Form state
  const [formData, setFormData] = useState({
    title: initialValues ? initialValues?.title : "",
    postedOn: initialValues ? initialValues?.postedOn : "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await _POST(
        `reviewmain/${editId ? `update?pol_id=${editId}` : "create"}`,
        formData,
        `${editId ? "PUT" : "POST"}`
      );
  ...
  return (
    <div className="max-w-4xl mx-auto my-8 p-6 bg-white rounded-lg shadow-md">
      <h1 className="text-2xl font-bold text-red-700 mb-6">Add a New Review</h1>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-2">
          <label
            ...
            className="block text-sm font-medium text-gray-700"
          >
            Posted On <span className="text-red-600">*</span>
          </label>
          <input
            type="date"
            id="postedOn"

...
function FeatureBlogTable({ Blogs }) {
  ...
  async function deleteItem(e, ID, setItems, items, key) {
    e.stopPropagation();
    e.preventDefault();
    const response = await _DELETE(`blog/remove?blog_id=${ID}`, 'core');
    if (response) {
            ...
            {blogList.map((index, id) => (
              ...
              >
                ...
                <td className="px-4 py-4 flex justify-between">
                  <Link href={`feature_blog/create/${index.blog_id}`}>

          ...
          <div className="divide-y divide-gray-200">
            {reviewerData.map((item, index) => (
              <div key={index} className="p-6 hover:bg-gray-50 transition duration-150">
                <h2 className="text-xl md:text-2xl font-bold text-red-800 mb-2">{item.title}</h2>
                <div className="text-sm text-gray-500 mb-3">
                  Posted on {isClient ? formatDate(item.date) : item.date.split(' ')[0]}
                </div>
                <p className="text-gray-700 mb-3">{item.content}</p>
                <Link href="#" className="inline-flex items-center text-red-700 hover:text-red-900 font-medium">
                  Read more 

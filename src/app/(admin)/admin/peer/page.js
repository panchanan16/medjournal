...
async function PeerPage() {
  const response = await _GET('review_process/readAll')
    ...
    <div className="h-screen flex flex-col py-6">
      <div className="relative overflow-y-auto focus:outline-none">
        <div className="py-6">
          ...
          <TitleAndContent type={response?.length ? 'PUT' : 'POST'} endpoints={`review_process/${response?.length ? `update?prp_id=${response[0]?.prp_id}` : 'create'}`} initialValues={response?.length ? response[0] : null} />
        </div>
      </div>
    </div>
  );
